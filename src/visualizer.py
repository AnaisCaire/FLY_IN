...
class Visualizer:
    def __init__(
        ...
        self.offset_x = (win_w - graph_w) // 2 - self.min_x * self.scale
        self.offset_y = hud_h + pad + (win_h - hud_h - 2 * pad - graph_h) // 2 \
    ...
    def _draw_connections(self) -> None:
        ...
        for connections in self.manager.adjacency_list.values():
            for conn in connections:
                key = tuple(sorted([conn.prev_zone.name, conn.next_zone.name]))
                if key in drawn:
                    continue
                drawn.add(key)
    ...
    def _draw_zones(self) -> None:
        for zone in self.manager.zone.values():
            ...
            pygame.draw.circle(self.screen, COLORS["white"], pos, self.zone_r, 2)
            ...
            if not (zone.is_start or zone.is_end):
                ...
                self.screen.blit(
                    cap_lbl,
                    (pos[0] - cap_lbl.get_width() // 2, pos[1] + self.zone_r + 14),
                )
    ...
    def run(self) -> None:
        ...
        while running:
            ...
            if now - last_advance >= TURN_DURATION_MS and current_turn < total - 1:
                current_turn += 1
                last_advance = now
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        running = False
                    if event.key == pygame.K_RIGHT and current_turn < total - 1:
                        current_turn += 1
                        last_advance = now
                    if event.key == pygame.K_LEFT and current_turn > 0:
                        current_turn -= 1
                        last_advance = now
